...
### CRT
C运行时库（CRT）是包含着ISO C99标准类的C++标准库的一部分，实现了CRT的Visual C++的类库支持原生代码开发，同时包括混合原生代码、托管代码、及.NET开发的纯托管代码。

### IME
编辑器输入法，这是一个程序，可以允许用户输入用户无法在他的键盘上无法找到的字符和符号。例如，他允许使用拉丁键盘的用户，输入中文、日文、韩文和印度字符。
...
这是一个共享类库，它包含了[Chromium内容模块]和相关的依赖库（比如 Blink, [V8] 等）。同时它指代名词 "libcc"。
...
V8 是谷歌公司的开源的 JavaScript 引擎。它使用 C++ 编写并使用在谷歌公司开源的的浏览器 Google Chrome 上。V8 能够独立运行，也可以集成在任何一个 C++ 应用内。
Electron 内置的 v8引擎，是作为 Chromium 的一部分存在的，当对它进行构建的时候，会把 Node 指向到 V8引擎。
V8的版本号经常和谷歌chrome的版本号是相呼应的。例如：Chrome 59 的 v8引擎 版本号就是V8.5.9，Chrome 58 的 V8引擎版本号就是 V8.5.8。
